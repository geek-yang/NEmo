...
class BBConvLSTMCell(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size, p=0.5):
        """
        Build convolutional cell for BBConvLSTM.
        ...
        """
        super(BBConvLSTMCell, self).__init__()
...
class BBConvLSTM(nn.Module):
    """
    This is the main BBConvLSTM module.
    param input_channels: number of channels (variables) from input fields
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size, p=0.5):
        super(BBConvLSTM, self).__init__()
        ...
        for i in range(self.num_layers):
            name = 'cell{}'.format(i)
            cell = BBConvLSTMCell(self.input_channels[i], self.hidden_channels[i], self.kernel_size, p)
            setattr(self, name, cell)
            self._all_layers.append(cell)        

    def forward(self, x, timestep):
        """
        Forward module of BBConvLSTM.
